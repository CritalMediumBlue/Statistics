...
**Remark (ordered data sequence)**
From a given data sequence $x_1, \ldots, x_n$, one sometimes builds an ordered data sequence $x_{(1)}, \ldots, x_{(n)}$. This is implemented in R by the command `sort()`.
...
**Remark (ECDF)**
A very important quantity for describing a data set of real numbers is its *empirical cumulative distribution function*. Given a data set $x_1, \ldots, x_n$, it is defined as the function $F_n : \mathbb{R} \rightarrow [0,1]$ where

$$F_n (t) = h_n ((- \infty, t]) = \frac{1}{n} \sum_{i = 1}^n 1_{(- \infty, t]} (x_i)$$ 

Here $1_{{}(- \infty, t]} (x)$ equals unity, if $x$ lies
in the interval $(- \infty, t]$, i.e., we have $x \leq t$, and equals
zero otherwise. Thus, the expression $F_n (t)$ is the relative frequency $h_n ((- \infty, t])$ of all $x_i$ satisfying $x_i \leq t$. Hence $t \mapsto F_n (t)$ is a monotonically increasing, piecewise constant function, which has jumps at $t =
x_i$, for $i \in \{ 1, \ldots, n \}$.
...
A plot of the empirical cumulative distribution function is very
instructive.
...
It is possible to calculate with vectors in the same way as with
numbers: Operations will be performed elementwise. This allows for very efficient coding.
...
**Definition (CDF)**
A real function $F : \mathbb{R} \rightarrow [0, 1]$ is called *cumulative distribution function*] if
...
**Note:**
A function $f : \mathbb{R} \rightarrow \mathbb{R}$ is *right continuous* in $x \in \mathbb{R}$, if for every sequence $(x_n)_{n \in \mathbb{N}}$, which converges to $x$ from the right, the sequence $(f (x_n))_{n \in \mathbb{N}}$ also converges to $f (x)$. A sequence $(x_n)_{n \in \mathbb{N}}$ converges to $x$ from the right, if $x_n > x$ for all $n$ and if the sequence $(x_n)_{n \in \mathbb{N}}$ converges to $x$.
